...
void InitKafka()
{
	std::string topics  = g_conf.getArgs("topics","deals");;//= g_conf.getArgs("");//"deals";
...
void InitMaxOffset()
{
	std::string sql_select = "select offset from kafka_deals order by  offset DESC limit 1;";
	...
	db.OpenDB();	
	g_offset = db.GetMaxOffset(sql_select);
	db.CloseDB();
}
