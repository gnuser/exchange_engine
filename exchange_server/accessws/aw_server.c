...
// static int on_method_rates_query(nw_ses *ses, uint64_t id, struct clt_info *info, json_t *params)
// {
//     // TODO
// 
//     sds key = sdsempty();
//     char *params_str = json_dumps(params, 0);
//     key = sdscatprintf(key, "%u-%s", CMD_RATES, params_str);
//     int ret = process_cache(ses, id, key);
//     if (ret > 0) {
//         sdsfree(key);
//         free(params_str);
...
//     memset(&pkg, 0, sizeof(pkg));
//     pkg.pkg_type  = RPC_PKG_TYPE_REQUEST;
//     pkg.command   = CMD_RATES;
//     pkg.sequence  = entry->id;
//     pkg.req_id    = id;
//     pkg.body      = params_str;
//     pkg.body_size = strlen(pkg.body);
// 
//     rpc_clt_send(rate, &pkg);
//     log_trace("send request to %s, cmd: %u, sequence: %u, params: %s",
//             nw_sock_human_addr(rpc_clt_peer_addr(rate)), pkg.command, pkg.sequence, (char *)pkg.body);
//     free(pkg.body);
// 
//     return 0;
// 
...
static int on_method_rates_subscribe(nw_ses *ses, uint64_t id, struct clt_info *info, json_t *params)
{
    // if (!info->auth)
    //     return send_error_require_auth(ses, id);

    rates_unsubscribe(ses);
    size_t params_size = json_array_size(params);
    if(params_size != 0)
        return send_error_invalid_argument(ses, id);
    if (rates_subscribe(ses) < 0)
        return send_error_internal_error(ses, id);

    send_success(ses, id);
    return send_notify(ses, "rates.update", init_rate);
}

static int on_method_rates_unsubscribe(nw_ses *ses, uint64_t id, struct clt_info *info, json_t *params)
{
    send_success(ses, id);
    return rates_unsubscribe(ses);
}
...
static int init_svr(void)
{
    ...
    ERR_RET_LN(add_handler("rates.subscribe",   on_method_rates_subscribe));
    ERR_RET_LN(add_handler("rates.unsubscribe", on_method_rates_unsubscribe));
    return 0;
}
