...
int send_error(nw_ses *ses, uint64_t id, int code, const char *message)
{
    ...
    json_object_set_new(reply, "err-code", json_integer(code));
    json_object_set_new(reply, "err-msg", json_string(message));
    json_object_set_new(reply, "status", json_string("error"));
    json_object_set_new(reply, "id", json_integer(id));
    uint64_t uint_timestamp = get_now_timestamp();
    json_object_set_new(reply, "ts", json_integer(uint_timestamp));
...
int send_success(nw_ses *ses, uint64_t id)
{
    ...
    json_object_set_new(result, "status", json_string("ok"));
    // json_object_set_new(result, "subed", json_string(ses->param_buf));
    uint64_t uint_timestamp = get_now_timestamp();
    json_object_set_new(result, "ts", json_integer(uint_timestamp));
...
// 把原始的ch转化为 类似 "kline.update" 的形式
static char* get_method(const char* ch)
{
   ...
   if(strcmp(method_pos,"kline") == 0)
       strcpy(ret,"kline.update");
   else if(strcmp(method_pos,"depth") == 0)
       strcpy(ret,"depth.update");
   else if(strcmp(method_pos,"trade") == 0)
   {
       if(strcmp(params_pos,"detail") == 0)
          strcpy(ret,"deals.update");
       else
          strcpy(ret,"today.update");
   }
   else
   {
        /*if(strcmp(params_pos,"pair") == 0)
            strcpy(ret,"bae_state.update");
        else*/
            strcpy(ret,"state.update");
   }
    free(tmp);
    return ret;
}
...
int send_notify(nw_ses *ses, const char *method, json_t *params)
{
    ...
    // 从json中取出ses->id对应的所有ch
    // 遍历ch与method比较
    // 把对应的in_ch传入
    sds ch_id = sdsfromlonglong(ses->id); 
    json_t* json_id_value = json_object_get(gvl_ch,ch_id);
    int pos = -1;
    for ( int i = 0; i < json_array_size(json_id_value); ++i)
    {
        const char* ch_method = json_string_value(json_array_get(json_id_value,i));
        char* ch_to_update = get_method(ch_method);
        if ( strcmp( ch_to_update,method) == 0 )
        {
            pos = i;
            free(ch_to_update);
            break;
    ...
    const char* ch_method = json_string_value(json_array_get(json_id_value,pos));
    if(ch_method == NULL)
    {
    ...
    if(strcmp(method,"kline.update") == 0)
        bae_send_notify_update_kline(ch_method,notify);
    else if( strcmp(method,"depth.update") == 0){
        bae_send_notify_update_depth(ch_method,notify);
    }else if( strcmp(method,"deals.update") == 0){
        bae_send_notify_update_deals(ch_method,notify);
    }else if(strcmp(method,"state.update") == 0 /*&& pst == PAIR*/){
        bae_send_notify_update_state_pair(ch_method,notify);
    }else if(strcmp(method,"today.update") == 0){
        bae_send_notify_update_trade(ch_method,notify);
    }else{
...
static int on_method_bae_state_query(nw_ses *ses, uint64_t id, struct clt_info *info, json_t *params)
{
    if (!rpc_clt_connected(marketprice))
        return send_error_internal_error(ses, id);
...
static int on_method_bae_state_subscribe(nw_ses *ses, uint64_t id, struct clt_info *info, json_t *params)
{
    bae_state_unsubscribe(ses);
    size_t params_size = json_array_size(params);
    for (size_t i = 0; i < params_size; ++i) {
        const char *market = json_string_value(json_array_get(params, i));
        if (market == NULL || strlen(market) >= MARKET_NAME_MAX_LEN)
            return send_error_invalid_argument(ses, id);
        if (bae_state_subscribe(ses, market) < 0)
            return send_error_internal_error(ses, id);
    }

    send_success(ses, id);
    for (size_t i = 0; i < params_size; ++i) {
        bae_state_send_last(ses, json_string_value(json_array_get(params, i)));
    }

    return 0;
}

static int on_method_bae_state_unsubscribe(nw_ses *ses, uint64_t id, struct clt_info *info, json_t *params)
{
    bae_state_unsubscribe(ses);
    return send_success(ses, id);
}
...
static int on_message(nw_ses *ses, const char *remote, const char *url, void *message, size_t size)
{
    ...
    json_t *in_msg = json_loadb(message, size, 0, NULL);
    if (in_msg == NULL) {
        goto decode_error;
    }
    
    // 做一个全局json去保存ses->id 对应的 方法
    ...
    // TODO
    sds sds_id = sdsfromlonglong(ses->id);
    if(type == SUB)
    {
        //json_t *ret_id = json_array();
        //json_array_append(ret_id,json_integer(ses->id));
        //for(int i=0; i<json_array_size(gvl_ch); ++i)
        //{
        //    if(json_array_get(gvl_ch,0) == ses->id)
        //    {
        //        json_array_append(ret_id,ret_sub);
        //    }
        //}
        //json_t *ret_sub = json_array();
        //json_array_append(ret_sub,json_string(ch_sub));

        //json_array_append_new(gvl_ch,ret_id);
        json_t *jsub = json_object_get(gvl_ch,sds_id);
        if (NULL == jsub)
        {
            json_t* jses_id_value = json_array();
            json_array_append_new(jses_id_value,json_string(ch_sub));
            json_object_set(gvl_ch,sds_id,jses_id_value);
        }
        else
        {
            json_array_append_new(jsub,json_string(ch_sub));
            json_object_set(gvl_ch,sds_id,jsub);
        }
    }
    else
    {
        ...
        if (NULL != jsub)
        {
            for(int i = 0; i<json_array_size(jsub); ++i)
            {
                if(strcmp(ch_sub,json_string_value(json_array_get(jsub,i))) == 0)
                {  
                    json_array_remove(jsub,i);
                    break;
                }
            }
        }
...
static void on_close(nw_ses *ses, const char *remote)
{
    ...
    // 在此处删除gvl_ch对应 id 的缓存
    sds sds_id = sdsfromlonglong(ses->id);
    json_object_del(gvl_ch,sds_id);
}
...
static int init_svr(void)
{
    ...
    ERR_RET_LN(add_handler("bae_state.query",       on_method_bae_state_query));
    ERR_RET_LN(add_handler("bae_state.subscribe",   on_method_bae_state_subscribe));
    ERR_RET_LN(add_handler("bae_state.unsubscribe", on_method_bae_state_unsubscribe));
