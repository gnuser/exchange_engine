...
int insert_market_to_db(uint32_t buy_id, uint32_t sell_id, double min_count)
{
    ...
    sds sql = sdsempty();
    sql = sdscatprintf(sql, "INSERT INTO system_trade_type (buy_coin_id, sell_coin_id, min_count, status) VALUES "
          "(%u, %u, %f, 1)", buy_id, sell_id, min_count);

    log_stderr("insert market to db");
    log_trace("exec sql: %s", sql);
    int ret = mysql_real_query(conn, sql, sdslen(sql));
    if (ret != 0) {
        log_error("exec sql: %s fail: %d %s", sql, mysql_errno(conn), mysql_error(conn));
