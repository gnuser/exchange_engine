...
static int on_cmd_market_update(nw_ses *ses, rpc_pkg *pkg, json_t *params)
{
    ...
    if (!json_is_integer(json_array_get(params, 0))) 
        return reply_error_invalid_argument(ses, pkg);
    uint32_t buy_id = json_integer_value(json_array_get(params, 0));

    if (!json_is_integer(json_array_get(params, 1))) 
        return reply_error_invalid_argument(ses, pkg);
    uint32_t sell_id = json_integer_value(json_array_get(params, 1));

    if (!json_is_real(json_array_get(params, 2))) 
        return reply_error_invalid_argument(ses, pkg);
    double min_count = json_real_value(json_array_get(params, 2));
    if (min_count <= 0.0)
        return reply_error_invalid_argument(ses, pkg);

    int ret = insert_market_to_db(buy_id, sell_id, min_count);
    if (ret < 0) {
        error(EXIT_FAILURE, errno, "market update1 fail: %d", ret);
    }

    ret = init_asset_and_market(true);
    if (ret < 0) {
        error(EXIT_FAILURE, errno, "market update2 fail: %d", ret);
    }
...
static int on_cmd_asset_update(nw_ses *ses, rpc_pkg *pkg, json_t *params)
{
    ...
    int ret = insert_asset_to_db(asset);
    if (ret < 0) {
...
static void svr_on_recv_pkg(nw_ses *ses, rpc_pkg *pkg)
{
    ...
    case CMD_BALANCE_SETTLE:
        ...
        log_trace("from: %s cmd balance settle, sequence: %u params: %s", nw_sock_human_addr(&ses->peer_addr), pkg->sequence, params_str);
